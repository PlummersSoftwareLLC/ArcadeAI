...
PREVIEW_WEBRTC_MAX_ACTIVE_PCS = 2
PREVIEW_WEBRTC_PC_STALE_S = 30.0


_FALLBACK_ICE_SERVERS = [{"urls": ["stun:stun.l.google.com:19302"]}]
_WEBRTC_ICE_SERVERS = _FALLBACK_ICE_SERVERS
...
def _repo_root() -> str:
    """Return the top-level repo directory (…/ai)."""
    return os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))


def _audio_dir() -> str:
    return os.path.join(_repo_root(), "audio")


def _fonts_dir() -> str:
    return os.path.join(_repo_root(), "fonts")


def _html_dir() -> str:
    return os.path.join(_repo_root(), "html")
...
class _DashboardState:
    def __init__(self, metrics_obj, agent_obj=None, history_limit: int = DASH_HISTORY_LIMIT):
        ...
        self._last_inf_request_count = 0
        self._last_inf_rate_time: float | None = None
        self._last_inf_req_per_sec = 0.0
        self._last_avg_expert_ms = 0.0
    ...
    def _get_model_desc(self) -> str:
        ...
        layers = [
            ...
            f"tx{cfg.transformer_layers}x{cfg.embed_dim}",
            f"g{cfg.global_context_dim}",
            f"ray{cfg.action_feature_dim}",
        ...
        self._model_desc = f"Model: ObjectRay+PPO \u00b7 {arch} \u00b7 {p_str} params"
        return self._model_desc
    ...
    def _get_model_summary_text(self) -> str:
        ...
        lines = [
            ...
            f"  Transformer layers / heads: {cfg.transformer_layers} / {cfg.num_heads}",
            f"  Action ray feature dim: {cfg.action_feature_dim}",
            "",
            "Policy",
    ...
    def _build_snapshot(self) -> dict[str, Any]:
        ...
        with m.lock:
            frame_count = int(m.total_frames)
            fps = float(sum(m.fps_window) / len(m.fps_window)) if m.fps_window else 0.0
            episode_rewards = list(m.episode_rewards)
            episode_lengths = list(m.episode_lengths)
            client_count = int(m.client_count)
            web_client_count = int(m.web_client_count)
            average_level = float(m.avg_level)
            peak_level = float(m.peak_level)
            peak_game_score = int(m.peak_game_score)
            avg_game_score = float(m.avg_game_score)
            game_count = int(m.total_games_played)
            episodes_this_run = int(m.episodes_this_run)
            game_preview_seq = int(m.game_preview_seq)
            game_preview_client_id = getattr(m, "game_preview_client_id", None)
            game_preview_width = int(m.game_preview_width)
            game_preview_height = int(m.game_preview_height)
            game_preview_format = str(m.game_preview_format)
            game_preview_fps = float(m.game_preview_fps)
            game_preview_source_format = str(m.game_preview_source_format)
            game_preview_encoded_bytes = int(m.game_preview_encoded_bytes)
            game_preview_raw_bytes = int(m.game_preview_raw_bytes)
            game_preview_compression_ratio = float(m.game_preview_compression_ratio)
            inference_requests = int(getattr(m, "total_inference_requests", 0) or 0)
            inference_time = float(getattr(m, "total_inference_time", 0.0) or 0.0)
            expert_requests = int(getattr(m, "total_expert_requests", 0) or 0)
            expert_time = float(getattr(m, "total_expert_time", 0.0) or 0.0)
        ...
        if self._last_inf_rate_time is not None:
            dt = max(1e-6, now - self._last_inf_rate_time)
            dr = max(0, inference_requests - self._last_inf_request_count)
            self._last_inf_req_per_sec = dr / dt
        self._last_inf_rate_time = now
        self._last_inf_request_count = inference_requests
        inf_req_per_sec = self._last_inf_req_per_sec
        if expert_requests > 0:
            self._last_avg_expert_ms = (expert_time / max(1, expert_requests)) * 1000.0
        avg_expert_ms = self._last_avg_expert_ms

        try:
            rwd100k, rwd1m, rwd5m = get_reward_window_averages()
        except Exception:
            rwd100k = rwd1m = rwd5m = 0.0
        level_25k, level_100k, level_1m, level_5m = self._update_level_windows(frame_count, average_level)

        epsilon = float(agent.get_epsilon()) if agent else 0.0
        expert_ratio = float(agent.get_expert_ratio()) if agent else 0.0
        bc_weight = float(agent._get_bc_weight()) if agent else 0.0
        lr = float(agent.optimizer.param_groups[0]["lr"]) if agent else 0.0
        policy_loss = float(agent.last_policy_loss) if agent else 0.0
        ...
        training_enabled = bool(agent.training_enabled) if agent else False
        override_expert = bool(agent.override_expert) if agent else False
        ...
        return {
            ...
            "inf_req_per_sec": inf_req_per_sec,
            "avg_expert_ms": avg_expert_ms,
            "epsilon": epsilon,
            ...
            "lr_warmup_steps": int(CONFIG.train.lr_warmup_steps),
            "lr_decay_steps": int(CONFIG.train.lr_decay_steps),
            "batch_size": int(CONFIG.train.mini_batch_size),
...
class _PreviewWebRTCBridge:
    def __init__(self, metrics_obj, ice_servers: list[dict[str, Any]] | None = None):
        ...
        self._loop = None
        self._pcs = set()
        self._pc_created_at: dict[Any, float] = {}
        self._pc_tracks: dict[Any, tuple[Any | None, Any | None]] = {}
    ...
    async def _close_pc_async(self, pc):
        if pc is None:
            return
        self._pcs.discard(pc)
        self._pc_created_at.pop(pc, None)
        tracks = self._pc_tracks.pop(pc, (None, None))
        for track in tracks:
            if track is None:
                continue
            try:
    ...
    async def _prune_pcs_async(self, max_active: int = PREVIEW_WEBRTC_MAX_ACTIVE_PCS):
        now = time.time()
        close_list = []
        live_entries: list[tuple[float, Any]] = []
        for pc in list(self._pcs):
            state = str(getattr(pc, "connectionState", "") or "").lower()
            created_at = float(self._pc_created_at.get(pc, now))
            is_terminal = state in {"closed", "failed", "disconnected"}
            is_stale = (now - created_at) >= PREVIEW_WEBRTC_PC_STALE_S and state != "connected"
            if is_terminal or is_stale:
                close_list.append(pc)
            else:
                live_entries.append((created_at, pc))

        if max_active >= 0 and len(live_entries) > max_active:
            live_entries.sort(key=lambda item: item[0], reverse=True)
            close_list.extend(pc for _created_at, pc in live_entries[max_active:])

        for pc in close_list:
            await self._close_pc_async(pc)

        if close_list:
            print(f"[v3] Pruned {len(close_list)} stale WebRTC preview connection(s)")
    ...
    async def _create_answer_async(self, offer_sdp: str, offer_type: str, profile: str = "default"):
        ...
        async with self._async_lock:
            await self._prune_pcs_async(max_active=max(0, PREVIEW_WEBRTC_MAX_ACTIVE_PCS - 1))

            pc = RTCPeerConnection(configuration=self._pc_configuration())
            self._pcs.add(pc)
            self._pc_created_at[pc] = time.time()
            video_track = None
            audio_track = None
            self._pc_tracks[pc] = (None, None)
            ...
            if profile_norm == "mobile":
                video_track = _PreviewVideoTrack(self.metrics, fps=12.0, max_width=160, max_height=132)
            else:
                profile_norm = "default"
                video_track = _PreviewVideoTrack(self.metrics, fps=30.0)
            ...
            self._pc_tracks[pc] = (video_track, audio_track)
...
<body>
  ...
  <main>
    <section class="cards">
      ...
      <article class="card card-half" style="--card-border:rgba(100,200,255,0.66);--card-glow:rgba(80,180,255,0.26)">
        <div class="label">AVG INFERENCE</div>
        <div class="value value-inline"><span class="metric-led" id="mInfLed"></span><span id="mInf">0.00 ms</span></div>
        <div class="record-row"><span class="record-label">Rate:</span><span class="record-value"><span id="mInfRate">0.0</span>/s</span></div>
      </article>
      <article class="card card-half" style="--card-border:rgba(255,190,100,0.66);--card-glow:rgba(255,165,60,0.26)">
        <div class="label">AVG EXPERT</div>
        <div class="value" id="mExpertMs">0.00 ms</div>
      </article>
      <article class="card card-half" style="--card-border:rgba(100,200,255,0.66);--card-glow:rgba(80,180,255,0.26)">
        <div class="label">ENTROPY</div>
        <div class="value" id="mEntropy">0.00</div>
      </article>
      <article class="card card-half" style="--card-border:rgba(220,180,255,0.66);--card-glow:rgba(200,150,255,0.26)">
        <div class="label">BC WEIGHT</div>
        <div class="value" id="mBcWeight">0.00</div>
      </article>
      <article class="card card-half" style="--card-border:rgba(255,220,100,0.66);--card-glow:rgba(255,200,80,0.26)">
        <div class="label">POLICY LOSS</div>
        <div class="value" id="mPolicyLoss">0.00</div>
      </article>
      <article class="card card-half" style="--card-border:rgba(200,100,255,0.66);--card-glow:rgba(180,80,255,0.26)">
        <div class="label">VALUE LOSS</div>
        <div class="value" id="mValueLoss">0.00</div>
      </article>
      <article class="card" style="--card-border:rgba(0,200,255,0.66);--card-glow:rgba(0,180,255,0.26)">
        <div class="label" style="display:flex;justify-content:space-between;align-items:center;">GAME SETTINGS</div>
        <div class="game-settings-row">
          <label>Level:
            <select id="gsLevel"></select>
          </label>
          <label style="gap:6px;">Advanced <span class="toggle-switch"><input type="checkbox" id="gsAdvanced"><span class="slider"></span></span></label>
        </div>
      </article>
    </section>
    ...
    <section class="charts">
      ...
      <article class="panel">
        ...
        <div class="legend">
          ...
          <span><span class="sw" style="background:#facc15;"></span>Avg</span>
        </div>
      ...
      <article class="panel" style="position:relative;">
        ...
        <div class="legend">
          <span><span class="sw" style="background:#00c8ff;"></span>Entropy</span>
  ...
  <script>
    ...
    const cards = {
      ...
      infRate: document.getElementById("mInfRate"),
      expertMs: document.getElementById("mExpertMs"),
      ...
      entropy: document.getElementById("mEntropy"),
      bcWeight: document.getElementById("mBcWeight"),
      policyLoss: document.getElementById("mPolicyLoss"),
      valueLoss: document.getElementById("mValueLoss"),
    ...
    const gsAutoCurrEl = null;  // V3 does not have auto-curriculum
    ...
    if (!_gsAdmin) { 
      ...
      if (gsAutoCurrEl) gsAutoCurrEl.disabled = true;
      if (chkPreviewEl) {
    ...
    const charts = {
      ...
      rewards: {
        ...
        series: [
          { key: "rwd_5m", color: "#38bdf8", median_window: 3,
            axis: {
              side: "left",
              label_pad: 52,
              group_keys: ["rwd_100k", "rwd_1m", "rwd_5m"],
            },
          },
          { key: "rwd_1m", color: "#22c55e", median_window: 3, axis_ref: "rwd_5m" },
          { key: "rwd_100k", color: "#ef4444", median_window: 3, axis_ref: "rwd_5m" },
          { key: "reward_avg", color: "#facc15", median_window: 5, smooth_alpha: 0.15,
            axis: {
              side: "right",
              label_pad: 40,
              group_keys: ["reward_avg"],
            },
          }
        ]
      },
      ...
      rewardMini: {
        ...
        series: [
          { key: "rwd_5m", color: "#38bdf8", median_window: 3, axis: { target_ticks: 3 } },
          { key: "rwd_1m", color: "#22c55e", median_window: 3 },
          { key: "rwd_100k", color: "#ef4444", median_window: 3, linearTime: true }
        ]
      },
      lossMini: {
      ...
      performance: {
        ...
        series: [
          { key: "entropy", color: "#00c8ff", smooth_alpha: 0.35,
            axis: { side: "left", min: 0, min_range: 0.50, label_pad: 52, group_keys: ["entropy"], tick_decimals: 2 }
          },
    ...
    function drawLrMiniChart(now) {
      ...
      const lrMax     = now.lr_max       || 3e-4;
      const lrMin     = now.lr_min       || 1e-5;
      const warmup    = now.lr_warmup_steps    || 5000;
      const total     = now.lr_decay_steps     || 500000;
      ...
      // Total steps we show: full decay range or up to step + some margin
      const totalShow = Math.max(total, step * 1.2);
      ...
      // Compute lr at a given step (mirrors V3 _build_lr_scheduler)
      function lrAt(s) {
        if (s < warmup) {
          const frac = Math.max(0.01, s / Math.max(1, warmup));
          return lrMax * frac;
        }
        const progress = Math.min(1.0, (s - warmup) / Math.max(1, total - warmup));
        const cosine = 0.5 * (1.0 + Math.cos(Math.PI * progress));
    ...
    function updateCards(now, smoothedSteps) {
      ...
      if (cards.inf) cards.inf.textContent = `${fmtFloat(now.avg_inf_ms || 0, 2)} ms`;
      if (cards.infRate) cards.infRate.textContent = fmtFloat(now.inf_req_per_sec || 0, 1);
      if (cards.expertMs) cards.expertMs.textContent = `${fmtFloat(now.avg_expert_ms || 0, 2)} ms`;
      setInfLed(now.avg_inf_ms || 0);
      ...
      cards.rwrd.textContent = fmtInt(now.rwd_1m || 0);
      ...
      cards.entropy.textContent = fmtFloat(now.entropy || 0, 3);
      cards.bcWeight.textContent = fmtFloat(now.bc_weight || 0, 3);
      cards.policyLoss.textContent = fmtFloat(now.policy_loss || 0, 4);
      cards.valueLoss.textContent = fmtFloat(now.value_loss || 0, 3);
    ...
    function render(payload) {
      ...
      drawChart(charts.performance.canvas, chartHistory60m, charts.performance.series, 60 * 60);
    ...
    class DashboardHandler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            ...
            if path == "/api/game_settings":
                gs = {"start_advanced": GAME_SETTINGS.start_advanced, "start_level_min": GAME_SETTINGS.start_level_min}

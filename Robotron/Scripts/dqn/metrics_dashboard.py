...
try:
    from .config import RL_CONFIG, plateau_pulser, PlateauPulser, game_settings, ROBOTRON_SELECTABLE_LEVELS
except ImportError:
    from config import RL_CONFIG, plateau_pulser, PlateauPulser, game_settings, ROBOTRON_SELECTABLE_LEVELS
...
def _parse_nvidia_smi_gpu_csv(text: str) -> list[dict[str, Any]]:
    """Parse the noheader,nounits CSV emitted by the dashboard GPU query."""
    rows = []
...
def _audio_dir() -> str:
    ...
    # Audio folder is at the workspace root (dqn is .../Robotron/Scripts/dqn).
    return os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(script_dir))), "audio")


def _fonts_dir() -> str:
    script_dir = os.path.dirname(os.path.abspath(__file__))
    return os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(script_dir))), "fonts")


def _html_dir() -> str:
    script_dir = os.path.dirname(os.path.abspath(__file__))
    # Optional static html under the game directory (.../Robotron/html).
    return os.path.join(os.path.dirname(os.path.dirname(script_dir)), "html")
...
class _DashboardState:
    ...
    def _get_model_desc(self) -> str:
        ...
        if object_count > 0 and object_features > 0:
            ...
            state_txt += (
                f" ({core_elist_features}g{lane_txt}{target_txt}{nearest_txt}+{object_count}x{object_features})"
            )
        attn_bits = []
    ...
    def _build_snapshot(self) -> dict[str, Any]:
        ...
        with self.metrics.lock:
            ...
            average_game_score = float(self.metrics.average_game_score)
            ...
            tactical_diag = dict(getattr(self.metrics, "tactical_diag_last", {}) or {})
...
<head>
  ...
  <style>
    ...
    *::before, *::after { box-sizing: border-box; }
    ...
    .live-average-item .record-label {
      margin-bottom: 3px;
    }
    .live-average-item .value {
      font-size: 18px;
      white-space: nowrap;
      overflow: hidden;
...
<body>
  ...
  <main>
    <section class="cards">
      ...
      <article class="card" style="--card-border:rgba(80,210,220,0.66);--card-glow:rgba(50,190,210,0.22)">
        <div class="label">LIVE AVERAGES</div>
        <div class="live-average-grid">
          <div class="live-average-item">
            <div class="record-label">Avg Score</div>
            <div class="value" id="mAvgGameScore">0</div>
          </div>
          <div class="live-average-item">
            <div class="record-label">Avg Level</div>
            <div class="value" id="mAvgGameLevel">0.0</div>
          </div>
        </div>
      </article>
      ...
      <article class="card" style="--card-border:rgba(60,220,170,0.66);--card-glow:rgba(40,200,150,0.22)">
        <div class="label">OBJECT WIRE</div>
        <div class="live-average-grid">
          <div class="live-average-item">
            <div class="record-label">Pool P/D/H/E</div>
            <div class="value" id="mObjPool">0/0/0/0</div>
          </div>
          <div class="live-average-item">
            <div class="record-label">Rows P/D/H/E</div>
            <div class="value" id="mObjRows">0/0/0/0</div>
          </div>
          <div class="live-average-item">
            <div class="record-label">Drop L/P</div>
            <div class="value" id="mObjDrop">0/0</div>
          </div>
          <div class="live-average-item">
            <div class="record-label">Active</div>
            <div class="value" id="mObjActive">0</div>
          </div>
        </div>
      </article>
    </section>
  ...
  <script>
    ...
    const GAUGE_MAX_FPS = 30000;
    const GAUGE_FPS_RED_MAX = 10000;
    const GAUGE_FPS_YELLOW_MAX = 20000;
    ...
    const cards = {
      ...
      avgGameScore: document.getElementById("mAvgGameScore"),
      avgGameLevel: document.getElementById("mAvgGameLevel"),
      ...
      objPool: document.getElementById("mObjPool"),
      objRows: document.getElementById("mObjRows"),
      objDrop: document.getElementById("mObjDrop"),
      objActive: document.getElementById("mObjActive"),
    };
    const gpuPanels = [
      {
        ...
        series: [
          {
            key: "fps",
            color: "#22c55e",
            axis: { side: "left", min: 0, max_floor: 3000, max_snap: 5000, tick_format: "compact", group_keys: ["fps", "eplen_100k"] },
            smooth_alpha: 0.14,
          },
          {
    ...
    function updateSystemPanels(now) {
      ...
      if (cpuPanel.name) cpuPanel.name.textContent = Number.isFinite(cpuCount) && cpuCount > 0 ? `System processor (${Math.round(cpuCount)} threads)` : "System processor";
      ...
      if (cpuPanel.loadCore) cpuPanel.loadCore.textContent = Number.isFinite(load1) && Number.isFinite(cpuCount) && cpuCount > 0
        ? (load1 / cpuCount).toFixed(2)
        : "—";

      const availGib = Number(now.ram_available_gib);
      const totalGib = Number(now.ram_total_gib);
      const usedGib = Number(now.ram_used_gib);
      const freePct = Number(now.ram_free_pct);
      if (ramPanel.live) ramPanel.live.textContent = Number.isFinite(availGib) ? `${availGib.toFixed(1)}G` : "--G";
      if (ramPanel.available) ramPanel.available.textContent = Number.isFinite(availGib) ? `${availGib.toFixed(1)}G` : "—";
      if (ramPanel.total) ramPanel.total.textContent = Number.isFinite(totalGib) ? `${totalGib.toFixed(1)}G` : "—";
      if (ramPanel.used) ramPanel.used.textContent = Number.isFinite(usedGib) ? `${usedGib.toFixed(1)}G` : "—";
      if (ramPanel.pct) ramPanel.pct.textContent = Number.isFinite(freePct) ? `${Math.round(freePct)}%` : "—";
    }
    ...
    function formatAxisTick(value, axis) {
      ...
      if (!Number.isFinite(n)) return "";
      if (axis && axis.tickFormat === "compact") {
        const a = Math.abs(n);
        if (a >= 1000000) {
          const v = n / 1000000;
          return `${Math.abs(v) >= 10 ? v.toFixed(0) : v.toFixed(1)}M`;
        }
        if (a >= 1000) {
          const v = n / 1000;
          return `${Math.abs(v) >= 10 ? v.toFixed(0) : v.toFixed(1)}K`;
        }
      }
      if (axis && Number.isFinite(axis.tickDecimals)) {
        return n.toFixed(axis.tickDecimals);
      }
      return Math.abs(n) >= 100 ? `${Math.round(n)}` : `${n.toFixed(0)}`;
    }
    ...
    function drawChart(canvas, history, seriesDefs, maxLookbackSec = (5 * 60), useLinearTime = false) {
      ...
      for (const s of axisSourceSeries) {
        ...
        axes.push({
          ...
          tickFormat: String(s.axis?.tick_format || ""),
        });
      }
      ...
      for (const axis of axes) {
        ...
        for (const tv of axis.ticks) {
          ...
          const labelText = formatAxisTick(tv, axis);
    ...
    function updateCards(now, smoothedSteps) {
      ...
      {
        const d = now.tactical_diag || {};
        const di = (key) => {
          const v = Number(d[key]);
          return Number.isFinite(v) ? String(Math.round(v)) : "0";
        };
        if (cards.objPool) {
          cards.objPool.textContent = `${di("lua_pool_projectile")}/${di("lua_pool_danger")}/${di("lua_pool_human")}/${di("lua_pool_electrode")}`;
        }
        if (cards.objRows) {
          cards.objRows.textContent = `${di("py_rows_projectile")}/${di("py_rows_danger")}/${di("py_rows_human")}/${di("py_rows_electrode")}`;
        }
        if (cards.objDrop) {
          cards.objDrop.textContent = `${di("lua_pool_dropped")}/${di("py_rows_clipped")}`;
        }
        if (cards.objActive) cards.objActive.textContent = di("py_rows_active");
      }
